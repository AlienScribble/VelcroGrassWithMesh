...
// This contains a lot of redundant draw methods - what I suggest is to make a backup of this file to extract draw methods as you need them
// That way you'll only ever have the methods you need. The reason I chose to keep them separate/specialized (rather than combined logic) is for customization and speed. 

namespace AlienScribble
{
    //FONTS : to create a font the surrounding colors must be green(0,255,0) with alpha space as 1 pixel and green seperations spaces as 1 pixel
    ...
    public class QuadBatch
    {
        ...
        private FontData[]   fd;
        private float        default_font_size = 0.6f;
        private Matrix world, view, proj, font_world, font_view, font_proj, ViewProj, WorldViewProj, font_WorldViewProj;

        // CONSTRUCTOR
        #region /// <summary>
        /// Construct QuadBatch. Need to pass QuadEffect.fx - set FontTextureFile to "" if not using quadBatch strings - set ScreenWidth, ScreenHeight to use a specific target resolution
        /// </summary> 
        /// <param name="content">               Content - used to load fx</param>
        /// <param name="graphics">              GraphicsDevice - gpu</param>
        /// <param name="Default_Effect_File">   Pass your own or the QuadEffect.fx (use SetEffect to change shaders at any time)</param>
        /// <param name="FontTextureFile">Use "" if not using quadbatch strings, otherwise specify a font texture</param>
        /// <param name="ScreenWidth">    target width of mainTarget - if null, it'll use the viewport settings</param>
        /// <param name="ScreenHeight">   target height of mainTarget - if null, it'll use the viewport settings</param>
        #endregion
        public QuadBatch(ContentManager content, GraphicsDevice graphics, string Default_Effect_File, string FontTextureFile, int? ScreenWidth, int? ScreenHeight)
        {
            ...
            default_shader = content.Load<Effect>(Default_Effect_File);   if (default_shader==null) { Log("Unable to load default shader"); }
            device = graphics;
            ...
            Use2DCamera();
            if (FontTextureFile.Length>1) PrepareFont(content.Load<Texture2D>(FontTextureFile));
        ...
        // Note: near the bottom there is a general DrawChained which is works for tails and hair with applied physics. 
        #region M E O  M O T I O N  C H A R A C T E R  D R A W   ( used in   M E O   P L A Y E R   or anything else with non-rectangular vertices)
        ...
        public void Draw(Rectangle sourceRect, Vector2 pos, float scale, float rot, Color color, SpriteEffects flip = SpriteEffects.None)
        {
            Draw(sourceRect, pos, null, new Vector2(scale, scale), rot, color, flip);
        }
        ...
        {
            depth = z_depth;
            Draw4Color(sourceRect, pos, origin, scale, rot, color, color, color, color, flip);
        }        
        //just rotates and scales (multicolor):                 
        ...
        {
            Draw4Color(sourceRect, pos, null, new Vector2(scale, scale), rot, c1, c2, c3, c4, flip);
        }
        ...
        {
            Draw4Color(sourceRect, pos, null, scale, rot, c1, c2, c3, c4, flip);
        }
        //assume origin is middle (offsets):        
        ...
        {
            DrawDistort4Color(sourceRect, pos, null, new Vector2(scale, scale), rot, x1off, y1off, x2off, y2off, x3off, y3off, x4off, y4off, c1, c2, c3, c4, flip);
        }
        ...
        {
            DrawDistort4Color(sourceRect, pos, null, scale, rot, x1off, y1off, x2off, y2off, x3off, y3off, x4off, y4off, c1, c2, c3, c4, flip);
        }
        ...
        #region D R A W   D I S T O R T   4   C O L O R
        public void DrawDistort4Color(Rectangle? sourceRect, Vector2 pos, Vector2? origin, Vector2? Scale, float rot, int x1off, int y1off, int x2off, int y2off, int x3off, int y3off, int x4off, int y4off, Color c1, Color c2, Color c3, Color c4, SpriteEffects flip = SpriteEffects.None)
        {
            if (!beginCalled) { Console.WriteLine("BEGIN not called before QuadBatch Draw. Draw aborted."); return; }
        ...
        #region D R A W   C O L O R   D I S T O R T  -  1 color version of distort:
        ...
        #region DRAW ENTIRE and DRAW DEPTH
        ...
        //-------------------
        #region D R A W - 4 colors (no distort - non-uniform scale)
        //-------------------
        public void Draw4Color(Rectangle? sourceRect, Vector2 pos, Vector2? origin, Vector2? _scale, float rot, Color c1, Color c2, Color c3, Color c4, SpriteEffects flip = SpriteEffects.None)
        {
            if (!beginCalled) { Console.WriteLine("BEGIN not called before QuadBatch Draw. Draw aborted."); return; }
            Vector2 scale = Vector2.One;
            float w, h, o_x, o_y, x1, y1, x2, y2, x3, y3, x4, y4, u1, v1, u2, v2;
        ...
        //----------------------
        #region D R A W  - Non-Uniform scale ( 1 color )
        ...
        //----------------------
        #region D R A W  - ( 1 color , float scale)
        ...
        //-----------------
        #region D R A W  1 Color  (very basic)
        ...
        //---------------------------------------
        #region D R A W   D E S T   4   C O L O R    (4 color & destinationRect)
        //---------------------------------------
        public void DrawDest4Color(Rectangle? sourceRect, Rectangle destRect, Color c1, Color c2, Color c3, Color c4, SpriteEffects flip = SpriteEffects.None)
        {
            if (!beginCalled) { Console.WriteLine("BEGIN not called before QuadBatch Draw. Draw aborted."); return; }
        ...
        #region D R A W   D E S T   (basic ones - draw to rectangle destination)
        public void DrawDest(Rectangle destRect, Color color, SpriteEffects flip = SpriteEffects.None)
        {
            DrawDest(null, destRect, color, flip);
        }
        // DRAW DEST 1 color:
        ...
        #region B E G I N -------------------------------------
        /// <summary>   Start a batch of rendering using a sprite sheet texture   </summary>        
        //BEGIN-----------------------------------------------------------------------------------------------------------
        ...
        {
            if (tex == null) { Console.WriteLine("QuadBatch Begin aborted because tex == null"); return; }
            if (beginCalled) { Console.WriteLine("QuadBatch Begin already called."); return; }
        ...
        public void End()
        {
            ...
            if (vert_count >= 3)
            {
                ...
                default_shader.Parameters["transform"].SetValue(WorldViewProj);
        ...
        #region C U S T O M   B I T M A P   F O N T   S T U F F 
        // **TO DO**: Improve font system - needs to be easier to understand, even faster, and provide flawless metrics - also provide 3D font support, bevels, 3D transforms, etc
        // P R E P A R E   F O N T                                    
        private float average_width;
        public float Average_Width { get { return average_width; } }
        ...
        {
            Vector2 size = new Vector2(1, fd[32].h * default_font_size); //float kern_space = text_h_space * 0.25f; float mult = default_font_size * 0.5f; // removed
            int a = 0;
            do
            {
                size.X += fd[(int)text[a]].w + fd[32].iw; //size.X += fd[(int)text[a]].w; // edited
                a++;
            } while (a < text.Length);
            size.X -= fd[32].iw; // added
            size.X *= default_font_size; //*= mult;  //size.X += (kern_space * text.Length) + text_h_space * 0.5f;
            return (size);
        }
        ...
        public Vector2 MeasureStringFast(string text)
        {
            ...
            float spacing = average_width * 1.1f * default_font_size;// + fd[32].iw;
            size.X = ((float)text.Length * spacing); //- fd[32].iw) * default_font_size;
            return (size);
        }
        ...
        // D R A W  S T R I N G
        /// <summary> creates a list of vertices and texture coordinates to use with the currently loaded font texture for message displays </summary>
        public void DrawString(string text, Vector2 pos, Vector2 scale, Color color, Alignment align = Alignment.None)
        {
            ...
            float spacing = (average_width * 1.1f) * scale.X;
        ...
        public void DrawStringFast(string text, Vector2 pos, Color color)
        {
            if (!beginCalled) { Console.WriteLine("BEGIN not called before Quadbatch Draw. Draw aborted."); return; }
            if (!font_tex_loaded) { Console.WriteLine("Font texture not loaded. Aborting DrawString."); return; }
            float px = pos.X + average_width * 0.5f * default_font_size; // + (average_width * 0.5f * default_font_size);
            float py = pos.Y;
            float spacing = (average_width*1.1f) * default_font_size;
        ...
        public void DrawStringFastScaled(string text, Vector2 pos, float scale, Color color)
        {
            ...
            float final_scale = default_font_size * scale;
            float spacing = (average_width * 1.2f) * final_scale;
        ...
        #region D R A W    L I N E s  &  R E C T s  (from lines) 
        ...
        #region D R A W   C O L O R   Q U A D 
        ...
        //***********************************************
        #region S P E C I A L I Z E D 
        //***********************************************

        //----------------------
        // D R A W   T R E E- ( 1 color , float scale)  [ for drawing things where only top 2 vertices rotate - like with a swaying tree ]
        //----------------------
        public Vector2 DrawTree(Rectangle? sourceRect, Vector2 pos, Vector2? origin, Vector2 scale, float rot, Color c1, SpriteEffects flip = SpriteEffects.None)
        {
            if (!beginCalled) { Console.WriteLine("BEGIN not called before QuadBatch Draw. Draw aborted."); return Vector2.Zero; }
            float w, h, o_x, o_y, x1, y1, x2, y2, x3, y3, x4, y4, u1, v1, u2, v2;
            if (!sourceRect.HasValue) sourceRect = new Rectangle(0, 0, tex.Width, tex.Height);
            if ((scale.X != 1f) || (scale.Y != 1f))
            {
            ...
            if ((flip & SpriteEffects.FlipVertically) != 0)
            {
                var temp = v2; v2 = v1; v1 = temp; //BR_Y                
            }
            if ((flip & SpriteEffects.FlipHorizontally) != 0)
            {
                var temp = u2; u2 = u1; u1 = temp; //BR_X                
            }
            vertices[vert_count].Position = new Vector3(x1, y1, depth); vertices[vert_count].TextureCoordinate = new Vector2(u1, v1);   //upper-left texCoord
            vertices[vert_count].Color = c1; vert_count++;
            vertices[vert_count].Position = new Vector3(x2, y2, depth); vertices[vert_count].TextureCoordinate = new Vector2(u2, v1);   //upper-right texCoord
            ...
            return new Vector2((x1 + x2) * 0.5f, (y1 + y2) * 0.5f);
        }// DrawTree



        // D R A W - WITH 2 COLOR --------- (used for drawing when top and bottom have a different shade of color)
        public void DrawWith2Color(Rectangle sourceRect, Vector2 pos, Vector2 origin, float rot, Color color1, Color color2, bool flip) //float squish, Color color, bool flip)
        {
            if (!beginCalled) { Console.WriteLine("BEGIN not called before QuadBatch Draw. Draw aborted."); return; } //if (tex == null) { Console.WriteLine("Somehow tex == null - aborting Quadbatch Draw."); return; } //extremely unlikely (since begin sets the tex only if tex is valid)            
            ...
            float x1, y1, x2, y2, x3, y3, x4, y4, tu1, tv1, tu2, tv2;

            w = sourceRect.Width; h = sourceRect.Height;
            o_x = origin.X; o_y = origin.Y;

            //need positions of destination                
            x1 = pos.X; y1 = pos.Y;         // + squish;
            x2 = pos.X + w; y2 = pos.Y;     // + squish;     //upper-right
            x3 = pos.X + w; y3 = pos.Y + h; //lower-right
            x4 = pos.X; y4 = pos.Y + h;     //lower-left            
            if (rot != 0f)
            {
            ...
            Rectangle tempRect = sourceRect;
            tu1 = tempRect.X / (float)tex.Width; //gets the texture coords in terms of (0.0f-1.0f, 0.0f-1.0f)
            tv1 = tempRect.Y / (float)tex.Height;
            tu2 = (tempRect.X + tempRect.Width) / (float)tex.Width;
            tv2 = (tempRect.Y + tempRect.Height) / (float)tex.Height;
            ...
            vertices[vert_count].Position = new Vector3(x1, y1, depth);
            vertices[vert_count].TextureCoordinate = new Vector2(tu1, tv1);   //upper-left texCoord
            vertices[vert_count].Color = color1; vert_count++;
            vertices[vert_count].Position = new Vector3(x2, y2, depth);
            vertices[vert_count].TextureCoordinate = new Vector2(tu2, tv1);   //upper-right texCoord
            vertices[vert_count].Color = color1; vert_count++;
            vertices[vert_count].Position = new Vector3(x3, y3, depth);
            vertices[vert_count].TextureCoordinate = new Vector2(tu2, tv2);   //lower-right texCoord
            vertices[vert_count].Color = color2; vert_count++;
            vertices[vert_count].Position = new Vector3(x4, y4, depth);
            vertices[vert_count].TextureCoordinate = new Vector2(tu1, tv2);   //lower-left texCoord
            vertices[vert_count].Color = color2; vert_count++;
            if ((vert_count + 1) >= 8180)
            {
                EndVerts();
        ...
        //----------------------
        // D R A W  - ( 1 color , float scale) [ returns the center endpoint at the top of a sprite - could be used with branches ]
        public Vector2 Draw_GetVec(Rectangle? sourceRect, Vector2 pos, Vector2? origin, float scale, float rot, Color c1, SpriteEffects flip = SpriteEffects.None)
        {
            if (!beginCalled) { Console.WriteLine("BEGIN not called before Quadbatch Draw. Draw aborted."); return Vector2.Zero; }
            float w, h, o_x, o_y, x1, y1, x2, y2, x3, y3, x4, y4, u1, v1, u2, v2;
            if (!sourceRect.HasValue) sourceRect = new Rectangle(0, 0, tex.Width, tex.Height);
        ...
        #region A D D   V E R T E X   (helper for DrawChained stuff)
        // For programming convenience, this separate method can be used to make code look more compact. If optimizing becomes necessary, maybe better to do directly. 
        void AddQuad(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4, float u1, float v1, float u2, float v2, Color color)
        {
            vertices[vert_count].Position = new Vector3(x1, y1, depth); vertices[vert_count].TextureCoordinate = new Vector2(u1, v1);   //upper-left texCoord
            vertices[vert_count].Color = color; vert_count++;
            vertices[vert_count].Position = new Vector3(x2, y2, depth); vertices[vert_count].TextureCoordinate = new Vector2(u2, v1);   //upper-right texCoord
            vertices[vert_count].Color = color; vert_count++;
            vertices[vert_count].Position = new Vector3(x3, y3, depth); vertices[vert_count].TextureCoordinate = new Vector2(u2, v2);   //lower-right texCoord
            vertices[vert_count].Color = color; vert_count++;
        ...
        #region C H A I N E D   ( HAIR, TAIL, ROPE, TENTACLE, CAPE, SNAKE, CHAIN, FLAG, HOSE, VINE )

        #region ///<summary>   D R A W  C H A I N E D    (for grass, tails, ropes, capes and other floppy stuff - can use VelcroPhysics to drive the motion) 
        ///         This version takes a single image source and warps it based on a set of vectors </summary>
        ///<param name="sourceRect"> Single image to warp (ie: a tail, hair, grass, etc) </param>                
        ///<param name="pivot_pos">  position is like the origin - this is what the chain pivots around (like the base of a tail) </param> 
        ///<param name="joints">     pass any number of Vector2 which represent the chained links/bones/joints: ie: tail_vec1, tail_vec2, tail_vec3  - note that these are relative vectors</param>
        ///<param name="scale_width">widen the image itself so it looks the way it should in relation to the physics</param>
        ///<param name="offset_base">use to stretch or squash the image at the base so it looks correct</param>
        ///<param name="offset_tip"> use to stretch or squash the image at the tip so it looks correct</param>       
        ///<param name="rotate_base">should the base rotate? ie: the bottom 2 vertices of grass do not rotate, but all others do. </param>        
        // Relative Vectors: example: 
        // pivot_pos = 360,600 and joints[0] = 10,0  _  joints[1]= 8,3  _  joints[2] = 4, 5  (these are added to 360,600 + 10,0 = 370,0 + 8,3 = 378,603 + 4,5 = 382,608 )
        // TO DO: optimizations - maybe even a vertex shader (altho probably not necessary) 
        #endregion
        public void DrawChainedGrass(Rectangle sourceRect, Vector2 pivot_pos, List<Vector2> joints, Color color,  
                                     float scale_width=1f, float h_off=0f, float tip_scale=0f, float base_scale=0f, 
                                     bool rotate_base = false, SpriteEffects flip = SpriteEffects.None) 
        {
            if (!beginCalled) { Console.WriteLine("BEGIN not called before QuadBatch Draw. Draw aborted."); return; }
            int num_joints = joints.Count; 
            float w, x1, y1, x2, y2, x3, y3, x4, y4, u1, v1, u2, v2;
            w = sourceRect.Width * scale_width;                           
            u1 =  sourceRect.X / (float)tex.Width;                  //gets the texture coords in terms of (0.0f-1.0f, 0.0f-1.0f)
            v1 =  sourceRect.Y / (float)tex.Height;
            u2 = (sourceRect.X + sourceRect.Width)  / (float)tex.Width;
            v2 = (sourceRect.Y + sourceRect.Height) / (float)tex.Height;
            float vdiv = (v2 - v1)/(float)num_joints;            
            if ((flip & SpriteEffects.FlipVertically)   != 0) {   var temp = v2; v2 = v1; v1 = temp;    }
            if ((flip & SpriteEffects.FlipHorizontally) != 0) {   var temp = u2; u2 = u1; u1 = temp;    }

            // for simplicity, set j = current bone/joint and uj = unit_vector of j (size 1)
            float hw = w * 0.5f, h_left, h_right;
            Vector2 j, uj, left, left_dir, tip_off, base_off, right;
            j        = joints[0];                 // get joint vector
            uj       = Vector2.Normalize(j);      // unit direction of joint

            base_off = uj * -base_scale;          // size of base offset    
            tip_off  = uj *  tip_scale;           // size of tip offset
            left_dir = new Vector2(uj.Y, -uj.X);  // unit direction of perpendicular (left) vector           
            h_left   = hw + h_off;                // horizontal distance (distance along perpendicular)  
            h_right  = hw - h_off;                // same for other size except offset is opposite
            left     =  left_dir * h_right;       // length is half-width (adjusted by horizontal offset)
            right    = -left_dir * h_left;        // same except the vector pointing to the right perpendicular to joint

            if (!rotate_base) {
                x3 = pivot_pos.X + h_left;        // pivot + left-offset
                x4 = pivot_pos.X - h_right;       // pivot - right-offset
                y3 = pivot_pos.Y;                 // remains welded to the base position
                y4 = pivot_pos.Y;                 // (get the base (bottom) - no rotation)
            } 
            else {
                x3 = pivot_pos.X + right.X;       // pivot + right(perpendicular) vector
                x4 = pivot_pos.X + left.X;        // pivot + left  "                   "  
                y3 = pivot_pos.Y + right.Y;       //               "                                 "
                y4 = pivot_pos.Y + left.Y;        //               "       "         (rotate the base)
            }
            x1 = x4 + j.X;     x2 = x3 + j.X;      y1 = y4 + j.Y;      y2 = y3 + j.Y;     // add the vector to the bottoms to get the tops           
            //DrawLine(pixel, new Vector2(x4, y4), new Vector2(x3, y3), Color.Red);
            x3 += base_off.X;  y3 += base_off.Y;   x4 += base_off.X;   y4 += base_off.Y;  // stretch the bottom out if needed
            //DrawLine(pixel, pivot_pos, pivot_pos + j, Color.White);
            //DrawLine(pixel, new Vector2(x1,y1), new Vector2(x2,y2), Color.Red);
            if (num_joints <= 1) {
                x1 += tip_off.X; y1 += tip_off.Y; x2 += tip_off.X; y2 += tip_off.Y;
                AddQuad(x1, y1, x2, y2, x3, y3, x4, y4, u1, v1, u2, v2, color);
                return;
            }

            // loop remaining bones: 
            float nx3, ny3, nx4, ny4, px = pivot_pos.X + j.X, py = pivot_pos.Y + j.Y, ox3 = x3, ox4 = x4, oy3 = y3, oy4 = y4;
            float v_div = (v2 - v1) / num_joints; 
            float pv1 = v2 - v_div, pv2 = v2;
            int i = 1;
            while(i<num_joints) {
                j        = joints[i];
                uj       = Vector2.Normalize(j);
                left_dir = new Vector2(uj.Y, -uj.X);
                left     =  left_dir * h_right;
                right    = -left_dir * h_left;
                nx3 = px + right.X;
                nx4 = px + left.X;
                ny3 = py + right.Y;
                ny4 = py + left.Y;
                //DrawLine(pixel, new Vector2(nx4,ny4), new Vector2(nx3,ny3), Color.Red);  DrawLine(pixel, new Vector2(px,py), new Vector2(px,py)+j, Color.White);

                // make our connecting vertices (average of last top and of this one's bottom)
                x4 = (nx4 + x1) / 2; y4 = (ny4 + y1) / 2;
                x3 = (nx3 + x2) / 2; y3 = (ny3 + y2) / 2;
                // use the old x1,y1,x2,y2 and ox3,oy3,ox4,oy4 to build the previous segment:
                AddQuad(x4, y4, x3, y3, ox3, oy3, ox4, oy4, u1, pv1, u2, pv2, color);
                pv1 -= v_div; pv2 -= v_div;
                //DrawLine(pixel, new Vector2(x4, y4), new Vector2(x3, y3), Color.Yellow);
                if (i >= num_joints - 1) {
                    tip_off = uj * tip_scale;
                    x1 = nx4 + j.X + tip_off.X;   x2 = nx3 + j.X + tip_off.X;   y1 = ny4 + j.Y + tip_off.Y;   y2 = ny3 + j.Y + tip_off.Y;
                }
                else {
                    x1 = nx4 + j.X; x2 = nx3 + j.X; y1 = ny4 + j.Y; y2 = ny3 + j.Y;
                }
                //DrawLine(pixel, new Vector2(x1, y1), new Vector2(x2, y2), Color.Red);
                px += j.X; py += j.Y;
                ox3 = x3; ox4 = x4; oy3 = y3; oy4 = y4;
                i++;
            }
            AddQuad(x1, y1, x2, y2, ox3, oy3, ox4, oy4, u1, pv1, u2, pv2, color);
        }//Draw (Non-Uniform)
